...
const createFormGenerationLinkGroup = ({
  autocomplete = false,
  autofocus = false,
  inputmode,
  label,
...
const createReasonField = (reasonData) => {
  const formReasonAttrs = { className: 'form-checkbox align-items-center', hidden: true }
...
export function createGenerateLinkForm () {
  const form = $('#form-generation-link')
  // Évite de recréer le formulaire s'il est déjà créé par react-snap (ou un autre outil de render)
  if (form.innerHTML !== '') {
    return
  }
  ...
  const formFirstPart = formData
    ...
    .filter(field => field.key !== 'datesortie')
    .filter(field => field.key !== 'heuresortie')
    .filter(field => !field.isHidden)
    .map((field,
      index) => {
      const formGroup = createFormGenerationLinkGroup({
        autofocus: index === 0,
        ...field,
        name: field.key,
      })
  ...
  const buttonGroup = createElement('p', { className: 'text-center mt-5' })
  const buttonAttrs = {
    className: 'btn btn-primary btn-attestation',
    id: 'generate-link-btn',
    type: 'submit',
  }
  const button = createElement('button', buttonAttrs)

  const span = createElement('span', { value: 'Générer mon lien automatique' })
  span.innerHTML = 'Générer mon lien automatique'
  const iElement = createElement('i', { className: 'fa fa-file-pdf inline-block mr-1' })

  const appendToButtonGroup = appendTo(buttonGroup)
  appendToButtonGroup(button)

  const appendToButton = appendTo(button)
  appendToButton(span)

  const appendToSpan = appendTo(span)
  appendToSpan(iElement)

  appendToForm([...createTitleGenerationLink(), ...formFirstPart, buttonGroup, reasonFieldset])
}
